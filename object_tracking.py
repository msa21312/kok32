...
xs = []
ys = []
video = cv2.VideoCapture("tuba.mp4")
...
def goal_track(img, bbox):
    ...
    c1 = x + int(w/2)
    c2 = y + int(h/2)
    cv2.circle(img,(c1,c2),2,(99,8,0),5)
    
    cv2.circle(img,(int(p1),int(p2)),2,(0,255,0),3)
    distancia = math.sqrt(((c1-p1)**2) + (c2-p2)**2)
    if(distancia<=20):
         cv2.putText(img,"Point",(75,90),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,255,0),2)
    xs.append(c1)
    ys.append(c2)
    for i in range(len(xs)-1):
        cv2.circle(img,(xs[i],ys[i]),2,(110,5,6),5)
        print('ponto')
